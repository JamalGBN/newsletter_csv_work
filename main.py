...
csv_file = "piano-pw_migration.csv"

sub_csv = "new_report.csv"

newsletter_csv = "joined_newsletters.csv"
...
read_csv(newsletter_csv, 'utf-8-sig', 'Emails', 'Newsletters', 'Newsletters', email_map, users)

count = 0

for i in range(len(users) -1, -1, -1):
    if (users[i]["Newsletters"] == ''):
        del users[i]

print(count)

# print(users[-1]["Newsletters"], "users")
